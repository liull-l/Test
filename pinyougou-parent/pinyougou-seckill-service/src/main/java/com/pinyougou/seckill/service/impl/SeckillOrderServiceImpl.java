...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	...
	public void submitOrder(Long seckillId, String userId) {
		...
		//扣减（redis）库存, 更新秒杀商品缓存（redis）
		seckillGoods.setStockCount(seckillGoods.getStockCount()-1);
		redisTemplate.boundHashOps("seckillGoods").put(seckillId,seckillGoods);//放回缓存
		if (seckillGoods.getStockCount()==0){//如果已经被秒光,1.同步到数据库,2.并清除此秒杀商品的redis缓存
	...
	public void saveOrderFromRedisToDb(String userId, Long orderId, String transactionId) {
		...
		seckillOrderMapper.insert(seckillOrder);//1.保存到数据
		redisTemplate.boundHashOps("seckillOrder").delete(userId);//从redis中清楚
	}
